from telegram import Update
from telegram.ext import ContextTypes

from src.bot.handlers.start import start_function_command
from src.bot.states import StateType

from src.bot.commands.text_generation.handlers import text_generation_main
from src.bot.commands.image_generation.main import image_generation_main
from src.bot.commands.info_of_nko.correct_info.correct_info_nko import show_current_info
#from src.bot.commands.text_generation.handlers import text_generation_main
from src.bot.commands.correct_text.main import *
from src.bot.commands.settings.main import *
from src.bot.commands.post_generation.main import *

async def handle_text_command_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    command = update.message.text.split()[0]
    '''Проверка, какая из команд текстового меню вызывана'''
    if command == "/text_generation":
        return await text_generation_main(update, context)
    elif command == "/image_generation":
        return await image_generation_main(update, context)
    elif command == "/correct_text":
        return await correct_text_main(update, context)
    elif command == "/post_generation":
        return await post_generation_main(update, context)
    elif command == "/plan":
        return await start_function_command(update, context) #это заглушка
    elif command == "/correct_info_nko":
        return await show_current_info(update, context)
    elif command == "/settings":
        return await settings_info(update, context)
    else:
        return StateType.MAIN_MENU